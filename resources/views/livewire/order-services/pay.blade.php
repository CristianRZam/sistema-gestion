<div class="p-6 bg-white dark:bg-zinc-900 rounded-lg shadow-md space-y-6">
    ...
    <div class="text-right space-y-2 text-gray-800 dark:text-gray-100">
        ...
        <div class="text-left md:text-right">
            ...
            @if($orden->estado_id !== 2 && $orden->estado_id !== 1)
                <input
                    type="number"
                    min="0"
                    step="0.01"
    ...
    @if($orden->estado_id !== 1)
        <div class="mt-4 flex flex-col md:flex-row md:justify-end items-stretch gap-2 md:gap-4">
            <flux:modal.trigger name="confirm-order-service-deletion">
                <flux:button class="w-full md:w-auto cursor-pointer" variant="danger" x-data="" x-on:click.prevent="$dispatch('open-modal-sale-delete', 'confirm-sale-deletion')">
                    {{ __('Cancelar servicio') }}
                </flux:button>
            </flux:modal.trigger>
        </div>
    @endif

    <flux:modal name="confirm-order-service-deletion" :show="$errors->isNotEmpty()" focusable class="max-w-lg">
        <form wire:submit="cancelarServicio" class="space-y-6">
            <div>
                <flux:heading size="lg">{{ __('¿Estás seguro de que deseas cancelar este servicio?') }}</flux:heading>

                <flux:subheading>
                    {{ __('Una vez cancelada, este servicio no podrá recuperarse. Esta acción es permanente. Confirma si deseas continuar.') }}
                </flux:subheading>
            </div>
...
<script>
    if (!window._cerrarModalCancelarServicio) {
        window._cerrarModalCancelarServicio = true;

        window.addEventListener('cerrarModalCancelarServicio', () => {
            setTimeout(() => {
                Flux.modal('confirm-order-service-deletion').close();
            }, 100);
        });
