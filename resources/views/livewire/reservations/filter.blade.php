        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div wire:ignore class="flex flex-col">
                <label for="estadoFiltroSelect" class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">
                    {{ __('Estado de reserva') }}
                </label>
                <div id="estadoFiltroSelect" class="w-full mt-1"></div>
            </div>

            {{-- Usuarios (virtual select) --}}
            <div wire:ignore class="flex flex-col">
                <label for="usuarioFiltroSelect" class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">
                    {{ __('Registrado por') }}
                </label>
                <div id="usuarioFiltroSelect" class="w-full mt-1"></div>
            </div>
        </div>
    ...
    <script>
        function initVirtualSelect(id, opciones, multiple = true) {
            ...
            el.addEventListener('change', function () {
                ...
                } else if (id === 'usuarioFiltroSelect') {
                    Livewire.dispatch('actualizarUsuariosDesdeJS', { valores: values });
                }
            });
        }

        window.initFiltrosVentas = function () {
        ...
        !!}];
            ...
            const usuarios = [{!!
            collect($usuarios)->map(fn($u) => "{ label: '".e($u->name)."', value: '".e($u->id)."' }")->implode(',')
        !!}];

            initVirtualSelect('estadoFiltroSelect', estados);
            initVirtualSelect('usuarioFiltroSelect', usuarios);
        };

        document.addEventListener('DOMContentLoaded', () => {
            window.initFiltrosVentas();
...
<script>
    document.addEventListener('livewire:navigated', function () {
        Livewire.on('limpiarFiltros', () => {
            ...
            document.querySelector('#usuarioFiltroSelect').reset();
        });
    });
