<flux:modal name="register-dish" :show="$errors->isNotEmpty()" focusable class="w-full max-w-3xl">
    <form wire:submit="guardar" class="space-y-6">
        <!-- Título -->
        <div>
            <flux:heading size="lg">
                {{ __('Agregar / Editar Platillo') }}
            </flux:heading>

            <flux:subheading>
                {{ __('Complete los campos para registrar o modificar un platillo.') }}
            </flux:subheading>
        </div>
...
<script>
    if (!window._cerrarModalDishRegistrado) {
        window._cerrarModalDishRegistrado = true;

        window.addEventListener('cerrarModalDish', () => {
            Flux.modal('register-dish').close();
            toastr.success('Registro guardado.');
        });
    }


    if (!window._abrirModalDish) {
        window._abrirModalDish = true;

        window.addEventListener('abrirModalDish', () => {
